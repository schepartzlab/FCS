...
%      number  tautriplet  taudiff1  taudiff2    A    alpha    G      s       
fp = [   20      0.1         0.6      0.3       0.5    0.6     0     0.2   ;     % initial values
         0       0.001       0.03       0.03        0     0.1   -.003   0.1    ;      % lower bounds
      10000     1.2         10      10       1      1     .003   0.5    ];     % upper bounds% End of recommended user edits.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

global s
s = fp(1,end);
fpstr =[{'number   '} 'tautriplet  ' 'taudiff1  ' 'taudiff2  ' '   A    '  ' alpha  '  '   G    ' '   s    ' '  Int   '];       % strings indicating the variables used
...
for k=1:datasets
    ...
    elseif inputdatasets > 0
        disp('Press spacebar to select a few curves (one dataset)'); pause
        filefilter = {'*.fcs','correlator files';'*.tsv','flexx files';'*.*','All files'};
    ...
    [name corrtimes corrdata inttimes intdata realdiscardlist tracenum filenumber j mode numcorr numint] = fcs_import_inspect(filelist,path,tau_begin,tau_end);
    
    % subtract 1
    corrdatasub=corrdata-1;
    r=size(corrdatasub,2);
    
    if k==1 % If this is the first runthru... 
    ...
    for i=1:mode
        corr_stdev(1:length(corrtimes),(i*2-1+(k-1)*mode*2) )= mean(corrdatasub(:,:,i), 2);
        if r>1
        ...
        corr_stdev(1:length(corrtimes),(i*2  +(k-1)*mode*2) )= stdtemp;
    end
    
    % now compute avgcorr and sd for the xcorr functions.
    if mode==3
        allxcorr=reshape(corrdatasub(:,:,3:4),length(corrtimes),[],1);
        corr_stdev(1:length(corrtimes),(5+(k-1)*mode*2) )= mean(allxcorr, 2);
        if r>1
        ...
        corr_stdev(1:length(corrtimes),(6+(k-1)*mode*2) )= stdtemp;
    end
    ...
    for i=1:mode
        agrnoise((i*4-3):i*4) = ...  agreement is StDev between curves, normalized by y.
            ...
            [ mean(corr_stdev(6:13,(i*2  +(k-1)*mode*2)))/mean(corr_stdev(6:13,(i*2-1+(k-1)*mode*2)))... at intercept (.03-.06ms)
            mean(corr_stdev(44:48,(i*2  +(k-1)*mode*2)))/mean(corr_stdev(44:48,(i*2-1+(k-1)*mode*2)))... at 1ms
            mean(corr_stdev(71:74,(i*2  +(k-1)*mode*2)))/mean(corr_stdev(71:74,(i*2-1+(k-1)*mode*2)))... at 10ms
            std(corr_stdev(32:42,(i*2-1+(k-1)*mode*2)))/mean(corr_stdev(32:42,(i*2-1+(k-1)*mode*2)))];
    ...
    stats(1:length(comment),k)=comment;
    
    % Plot average curves
    if mode==1
        % Plot final avgcorr and intensity
        hy=subplot(4,1,[1 3]);semilogx(corrtimes,corr_stdev(:,(1*2-1+(k-1)*mode*2)))
        ylabel('G(\tau) Channel A'),xlabel('\tau (ms)')  % this subplot is corrdata
        fullinta=reshape(intdata(:,:,1),[],1);  % produce the fullint
        hz=subplot(4,1,4); plot(1/tracenum:1/tracenum:(filenumber-j+1),fullinta)
        %  hz is the intensity subplot handle
        ylabel('Intensity, Channel A'),xlabel('FileNumber')
        set(hz,'XGrid','on')
    elseif mode==2
        % Plot final avgcorr and intensity
        hy=subplot(4,2,[1 5]);semilogx(corrtimes,corr_stdev(:,(1*2-1+(k-1)*mode*2)))
        ylabel('G(\tau) Channel A'),xlabel('\tau (ms)')  % this subplot is corrdata
        he=subplot(4,2,[2 6]);semilogx(corrtimes,corr_stdev(:,(2*2-1+(k-1)*mode*2)))
        ylabel('G(\tau) Channel B'),xlabel('\tau (ms)')  % this subplot is corrdata
        fullinta=reshape(intdata(:,:,1),[],1);  % produce the fullint
        fullintb=reshape(intdata(:,:,2),[],1);
        hz=subplot(4,2,7); plot(1/tracenum:1/tracenum:(filenumber-j+1),fullinta)
        %  hz is the intensity subplot handle
        ylabel('Intensity, Channel A'),xlabel('FileNumber')
        set(hz,'XGrid','on')
        hf=subplot(4,2,8); plot(1/tracenum:1/tracenum:(filenumber-j+1),fullintb)
        %hz is the intensity subplot handle
        ylabel('Intensity, Channel B'),xlabel('FileNumber')
        set(hf,'XGrid','on')
    elseif mode==3
        % Plot final avgcorr and intensity
        hy=subplot(4,3,[1 7]);semilogx(corrtimes,corr_stdev(:,(1*2-1+(k-1)*mode*2)))
        ylabel('G(\tau) Channel A'),xlabel('\tau (ms)')  % this subplot is corrdata
        he=subplot(4,3,[2 8]);semilogx(corrtimes,corr_stdev(:,(2*2-1+(k-1)*mode*2)))
        ylabel('G(\tau) Channel B'),xlabel('\tau (ms)')  % this subplot is corrdata
        fullinta=reshape(intdata(:,:,1),[],1);  % produce the fullint
        fullintb=reshape(intdata(:,:,2),[],1);
        hz=subplot(4,3,10); plot(1/tracenum:1/tracenum:(filenumber-j+1),fullinta)
        %  hz is the intensity subplot handle
        ylabel('Intensity, Channel A'),xlabel('FileNumber')
        set(hz,'XGrid','on')
        hf=subplot(4,3,11); plot(1/tracenum:1/tracenum:(filenumber-j+1),fullintb)
        %hz is the intensity subplot handle
        ylabel('Intensity, Channel B'),xlabel('FileNumber')
        set(hf,'XGrid','on')
        pa=subplot(4,3,[3 6]);semilogx(corrtimes,corr_stdev(1:length(corrtimes),(5+(k-1)*mode*2) ))
        pb=subplot(4,3,[9 12]);semilogx(corrtimes,corr_stdev(1:length(corrtimes),(5+(k-1)*mode*2) ));cla(pb)
    end
    ...
    elseif eqnmode>0
        ...
        for i=1:mode % loop for fitting mutiple times per dataset (dual or quad mode)
            ...
            weighted_residuals = @(x) (diffequation(x,corrtimes)-corr_stdev(1:length(corrtimes),(i*2-1+(k-1)*mode*2) ))... this term is avgcorr
                ./corr_stdev(1:length(corrtimes),(i*2  +(k-1)*mode*2) ); % this term is stdev
            ...
            figure
            subplot(4,1,[1 3]), semilogx(corrtimes,corr_stdev(1:length(corrtimes),(i*2-1+(k-1)*mode*2) ),'.b',corrtimes,fitdata,'-r')
            infobox{1} = cell2mat(fpstr);
            infobox(2) = {num2str(result(resrow,:),'%-12.4g')};
            infobox(3) = {['Fitting from ' name]};
            annotation('textbox', [.5 .6 .4 .3], 'String', infobox,'interpreter','none','FitBoxToText','on');
            ylabel('G(\tau)')
            title(['Correlation ' num2str(i)])
            subplot(4,1,4), semilogx(corrtimes,(corr_stdev(1:length(corrtimes),(i*2-1+(k-1)*mode*2) )-fitdata)./corr_stdev(1:length(corrtimes),(i*2  +(k-1)*mode*2) ),'.',corrtimes,0,'-r')
            xlabel('\tau (ms)')
            ylabel('Fitting Residuals (\sigma)')
            
            % Write a FitData file for this interval.
            alldata=[corrtimes corr_stdev(1:length(corrtimes),(i*2-1+(k-1)*mode*2)) corr_stdev(1:length(corrtimes),(i*2+(k-1)*mode*2) ) fitdata];
            data=[path 'FitData_mode_' num2str(eqnmode) '_' name 'corr' num2str(i) '.txt'];
    ...
    names1{k}=name;
    for h=1:mode
        names2{1+(k-1)*mode*2+(h-1)*2}=name;
        names2{2+(k-1)*mode*2+(h-1)*2}='sd';
        names4 = [names4 name];
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%fcs_import_inspect
function [name corrtimes corrdata inttimes intdata realdiscardlist tracenum filenumber j mode numcorr numint] = fcs_import_inspect(filelist,path,tau_begin,tau_end)

[filenumber]=size(filelist,2);
sinname='(-|_)(\w\w|\w)(.sin|.SIN|.fcs)';
name1=regexp(filelist{1},sinname);
name=filelist{1}(1:name1);
if isempty(name);
    name=filelist{1}(1:end-4);
end
disp(' ')
...
data_file = [path filelist{1}];
fullsinfile=textscan(fopen(data_file),'%s','Delimiter','\n');
fclose('all');
...
corrtimes = dlmread(data_file,',', [range1begin 0 range1end 0]);
inttimes = dlmread(data_file,',', [range2begin 0 range2end 0]);
% Change time units from seconds to milliseconds.

    
% Get corrdata and intdata from each file.
corrdata=zeros(size(corrtimes,1),filenumber,1);
intdata=zeros(size(inttimes,1),filenumber,1);
for i=1:filenumber
...
% mode-dependent plotting - plot 1, 2, or 4 correlation functions.
    ...
    fullinta=reshape(intdata(:,:,1),[],1);  % produce the fullint
    hz=subplot(4,1,4); plot(1/tracenum:1/tracenum:filenumber,fullinta)
    %hz is the intensity subplot handle
    ylabel('Intensity, Channel A'),xlabel('FileNumber')
    set(hz,'XGrid','on')
...
for j=1:filenumber
    ...
    if discard1~=0
        ...
        realdiscardlist(j)=realdiscard; %#ok<AGROW>
            ...
            legend('show'),ylabel('G(\tau) Channel A'),xlabel('\tau (ms)')  % this subplot is corrdata
            fullinta=reshape(intdata(:,:,1),[],1);  % produce the fullint
            axes(hz);subplot(4,1,4); plot(1/tracenum:1/tracenum:(filenumber-j),fullinta)
            %  hz is the intensity subplot handle
            ylabel('Intensity, Channel A'),xlabel('FileNumber')
            set(hz,'XGrid','on')
